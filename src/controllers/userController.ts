...
class UserController {
    ...
    async forgotPassword(req: express.Request, res: express.Response) {
        ...
        const link = `${process.env.CLIENT_URL}/user/reset-password?token=${token}&id=${user.id}`;
        try {
    ...
    async changePassword(req: express.Request, res: express.Response) {
        ...
        const user = await userService.findUserByQuery({ id: userId });
    ...
    async updateUserInfo(req: express.Request, res: express.Response) {
        const user = await userService.findUserByQuery({ id: req.body.id as string });
    ...
    async getUserInfo(req: express.Request, res: express.Response) {
        ...
        if (currentUser && userInfo && (currentUser.type === USER_TYPE.SUPER || userInfo.id === currentUserId)) {
            return res.status(200).json({
                id,
                bio: userInfo.bio,
    ...
    async manageUserData(req: express.Request, res: express.Response) {
        const {state, bio, removePhoto, filename, id, type} = req.body;
        const userIdToChange = id;
        const superUserId = req.headers.id;
        const superUser = await userService.findUserByQuery({ id: superUserId as string });
        const userToChange = await userService.findUserByQuery({ id: userIdToChange as string });
    ...
    async followUser(req: express.Request, res: express.Response) {
        ...
        const userToFollow = await userService.findUserByQuery({id: follow});
    ...
    async getUsersBy(req: express.Request, res: express.Response) {
        ...
        const userId = req.headers.id as string;
        const currentUser = await userService.findUserByQuery({id: userId as string}) as UserModel;
